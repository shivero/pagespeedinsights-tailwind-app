...
import { ItemType } from '../../types/ItemsSchema';
import { HeadingsSchema, HeadingsType, UserStore } from '../../types/LongTasksSchema';
const LcpLazyLoadedDetails = ({ audit }: { audit: LcpLazyLoadedType }) => {
    ...
    if (audit.details.items[0].node) {
        sanitizedSnippet = createdompurify.sanitize(audit.details.items[0].node.snippet);
    }
    return (
                ...
                <thead>
                    <tr className='bg-slate-100'>
                        {audit.details.headings.map((heading, index) => {
                            return (
                                <>
                                    <td className="text-sm p-1">{heading.label}</td>
                                </>
                            )
                        }
                        )}
                        ...
                        return (
                            <tr key={index}>
                                {audit.details.headings.map((heading, index) => {
                                    ...
                                    return (
                                        <>
                                            <td className='text-xs border border-b-1 p-1'><>{item[key] === 'node' ? <NodeComponent node={item[key]} /> : item[key]} {heading.valueType}</></td>
                                        </>
                                    )
                                })
                                }
